...
- 2026/08/15 — **v0.6.0**（相手ごとの「無視」とリアクション）
  - 参加者リストのハートで、その人の音を受け取らないようにできる（相手には伝わりません）
  - 定型リアクション8つ。押した人のアバターから浮かび、音も鳴る（切れます）
  - 「退出した」と「接続が切れた」を書き分けるようにした
- 2026/08/14 — **v0.5.1**（静かに壊れていたものを直した回）
  - 繋がらなかった接続を閉じるようにした（溜まり続けてブラウザの上限に当たっていた）
  - TURN 資格情報の寿命を4時間に（30分だと通話が突然切れていた）
  - シグナリングのリレーを実測で入れ替え（5本中4本が死んでいた）
